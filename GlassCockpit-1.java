...
public class GlassCockpit extends Container implements IObserver  {
    ...
    public GlassCockpit(){
        ...
        damageLvl = new SevenSegmentDisplay(0,2,ColorUtil.GREEN);
    ...
    public void update(ArrayList<GameObject> newGameworld) {
        ...
        for(GameObject currObj : gameObjects){
            if(currObj instanceof Helicopter){
                fuel = ((Helicopter)currObj).getFuelLevel();
                last = ((Helicopter)currObj).getLastSkyScraperReached();
                currHeading = ((Helicopter)currObj).getHeading();
                lives = ((Helicopter)currObj).getGameLife();
                damage = ((Helicopter)currObj).getDamageLevel();
                break;
            }
        }
        if(damage >= 85){
